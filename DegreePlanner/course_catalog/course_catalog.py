...
sql_str = (
    "SELECT * FROM EVENT WHERE " + "EVENT_STATUS = 'A' " + "AND EVENT_TYPE = 'CLAS' "
)
...
dfe = dfe.rename(
    columns={"EVENT_LONG_NAME": "course_name", "DESCRIPTION": "description"}
)
...
dfs = dfs.sort_values(
    ["EVENT_ID", "EVENT_SUB_TYPE", "ACADEMIC_YEAR", "ACADEMIC_TERM", "SECTION"],
...
dfcat = dfs.sort_values(
    ["EVENT_ID", "EVENT_SUB_TYPE", "CREDITS", "ACADEMIC_YEAR", "ACADEMIC_TERM"],
    ascending=[True, True, True, True, False],
)
dfcat = dfcat.drop_duplicates(["EVENT_ID", "EVENT_SUB_TYPE", "CREDITS"], keep="first")

dfcat = dfcat.rename(
    columns={"CREDITS": "min_credit_hours", "CIP_CODE": "course_cip_code"}
)
...
df.loc[(df["EVENT_SUB_TYPE"] == "LAB"), "course_name"] = df["course_name"] + " Lab"
...
df.loc[:, "ACADEMIC_YEAR"] = df.loc[:, "ACADEMIC_YEAR"].fillna(sections_begin_year)
df.loc[:, "ACADEMIC_TERM"] = df.loc[:, "ACADEMIC_TERM"].fillna("FALL")

cat_yr = (
    lambda c: c["ACADEMIC_YEAR"]
    if (c["ACADEMIC_TERM"] == "FALL")
...
df = df.sort_values(
    ["integration_id", "min_credit_hours"], ascending=[True, False]
